...
|          テスト対象         | 前提条件 |    操作    |         期待される挙動     | テスト結果 |
| -------------------------- | -------- | -------- | -------------------------- | -- |
| チケット種別の登録（成功例 1）  | 特になし | '1'を入力 | '通常'として登録できること | OK |
| チケット種別の登録（成功例 2）  | 特になし | '2'を入力 | '特別'として登録できること | OK |
| チケット種別の登録（失敗例 1）  | 特になし | '0'を入力 | '指定外の数字は入力しないでください。'として登録を拒否されること | OK |
| チケット種別の登録（失敗例 2）  | 特になし | '3'を入力 | '指定外の数字は入力しないでください。'として登録を拒否されること | OK |
