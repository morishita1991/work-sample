...
| 37 | 割増方法の登録（成功例-1）  | なし | '0'を入力 | '次へ進む'として登録できること | OK |
| 38 | 割増方法の登録（成功例-2）  | なし | '1'を入力 | '休日料金'として登録できること | OK |
| 39 | 割増方法の登録（失敗例-1）  | なし | '2'を入力 | '指定外の数字は入力しないでください。'として登録を拒否されること | OK |
| 40 | 割増方法の登録（失敗例-2）  | なし | '-１'を入力 | '指定外の数字は入力しないでください。'として登録を拒否されること | OK |
| 41 | 割増方法の登録（失敗例-3）  | なし | '１'を入力 | '半角数字で入力してください。'として登録を拒否されること | OK |
| 42 | 割増方法の登録（失敗例-4）  | なし | 'いち'を入力 | '半角数字で入力してください。'として登録を拒否されること | OK |
| 43 | 割増方法の再登録の確認（成功例-1）  | なし | '1'を入力 | 'はい'として受領され、再度割増方法の入力ができること | OK |
| 44 | 割増方法の再登録の確認（成功例-2）  | なし | '2'を入力 | 'いいえ'として受領され、次へ進む | OK |
| 45 | 割増方法の再登録の確認（失敗例-1）  | なし | '0'を入力 | '指定外の数字は入力しないでください。'として登録を拒否されること | OK |
| 46 | 割増方法の再登録の確認（失敗例-2）  | なし | '3'を入力 | '指定外の数字は入力しないでください。'として登録を拒否されること | OK |
| 47 | 割増方法の再登録の確認（失敗例-3）  | なし | '１'を入力 | '半角数字で入力してください。'として登録を拒否されること | OK |
| 48 | 割増方法の再登録の確認（失敗例-4）  | なし | 'いち'を入力 | '半角数字で入力してください。'として登録を拒否されること | OK |
